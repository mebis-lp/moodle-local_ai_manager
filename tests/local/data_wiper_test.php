...
final class data_wiper_test extends \advanced_testcase {
    /** @var stdClass The user object of our test user. */
     ...
     *
     * @covers data_wiper::anonymize_request_log_record
     */
     ...
     * @covers \local_ai_manager\local\data_wiper::anonymize_data
     * @covers \local_ai_manager\local\data_wiper::delete_data
     */
    public function test_anonymize_data_and_delete_data(): void {
        global $DB;
        $this->resetAfterTest();
        $course = $this->getDataGenerator()->create_course();
        $page = $this->getDataGenerator()->create_module('page', ['course' => $course->id]);
        ...
        $chatblock = $this->getDataGenerator()->create_block('ai_chat', ['parentcontextid' => $course->context->id]);
        $chatblockinstance = block_instance('ai_chat', $chatblock);
        $blockcontext = $chatblockinstance->context;
    ...
    /**
     * Tests the randomize string function.
     *
     * @covers \local_ai_manager\local\data_wiper::randomize_string
     */
    /*public function test_randomize_string(): void {
        $string = 'That is a test string including an utf-8 emoji 👋';
        $randomized = data_wiper::randomize_string($string);
        $this->assertNotEquals($string, $randomized);
        $this->assertEquals(mb_strlen($randomized, 'UTF-8'), mb_strlen($string, 'UTF-8'));
    }*/
    ...
    private function get_request_log_entries(): array {
        global $DB;
        return $DB->get_records('local_ai_manager_request_log');
    }
