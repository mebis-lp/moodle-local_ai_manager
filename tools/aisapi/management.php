...
use aitool_aisapi\form\management_form;
...
$PAGE->set_url('/local/ai_manager/tools/aisapi/management.php');
...
if ($managementform->is_cancelled()) {
    ...
    try {
        ...
        throw new \moodle_exception('err_apiresult', 'aitool_aisapi', '', $exception->getMessage());
    }
    if ($response->getStatusCode() === 200) {
        $usagereturn = $response->getBody()->getContents();
    } else {
        throw new \moodle_exception('err_apiresult', 'aitool_aisapi', '',
                get_string('statuscode', 'aitool_aisapi') . ': ' . $response->getStatusCode() . ': ' .
    ...
    if ($response->getStatusCode() === 200) {
        ...
        throw new \moodle_exception('err_apiresult', 'aitool_aisapi', '',
                get_string('statuscode', 'aitool_aisapi') . $response->getStatusCode() . ': ' . $response->getReasonPhrase());
    }
    ...
    echo $OUTPUT->render_from_template('aitool_aisapi/management',
            [
